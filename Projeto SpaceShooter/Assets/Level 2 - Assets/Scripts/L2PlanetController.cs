...
public class L2PlanetController : MonoBehaviour {
	...
	void MovePlanetDown () {
		...
		aPlanet.GetComponent<L2Planet>().isMoving = true;
	}
	...
	void EnqueuePlanets () {
		foreach (GameObject aPlanet in Planets) {
			...
			if ((aPlanet.transform.position.y < 0) && (!aPlanet.GetComponent<L2Planet>().isMoving)) {
				//reseta a posição do planeta
				aPlanet.GetComponent<L2Planet>().ResetPosition();
